...
| Microphone and audio | Not collected by an implemented product feature. | No audio capture or microphone workflow exists. | No app storage or provider processing. | Not applicable. | No microphone prompt or in-app control. | PF-204 Expo config introspection confirms no iOS microphone usage description and an Android manifest removal rule for `RECORD_AUDIO`. |
